  ...
  (e) => {
    ...
    if (e.touches.length == 2) {
      ...
      if (lastTouchDistance !== null) {
        ...
        const rect = app.view.getBoundingClientRect();
        const localMidpoint = container.toLocal(
          new PIXI.Point(
            screenMidpoint.x - rect.left,
            screenMidpoint.y - rect.top,
          ),
        );
...
PIXI.Assets.load([
  ...
  function initializeWebSocket(url) {
    ...
    ws.onmessage = function (event) {
      ...
      if ("stats" in data) {
        curStats = data["stats"];
        document.getElementById("envsCount").innerText =
          `${spriteCache.size} Environments Streaming`;
        document.getElementById("viewersCount").innerText =
          `${curStats.viewers} Viewers Connected`;
      } else {
  ...
  let baseTextureChar = new PIXI.BaseTexture(
    "assets/characters_transparent.png",
    {
      scaleMode: PIXI.SCALE_MODES.NEAREST,
    },
  ...
  function startAnimationForPath(path, meta) {
    ...
    if (meta && meta.user !== undefined && typeof meta.user === "string") {
      ...
      const color =
        meta.color && CSS.supports("color", meta.color)
          ? meta.color
          : "0x000000";
      ...
      if (userFilter.exec(labelText) !== null) {
        ...
        if (spriteCache.has(cacheKey)) {
          ...
          cached.batchTimingAvg = updateMovingAverage(
            cached.batchTimingAvg,
            timeSinceLastUpdate,
          );
          ...
          cached.animationDuration = Math.max(
            1000,
            cached.batchTimingAvg * 0.95,
          ); // 95% of avg to finish slightly early
          ...
          if (cached.spriteIdx !== spriteIdx) {
            ...
            cached.sprite.texture =
              directionTextures[cached.currentDirection][spriteIdx];
          }
